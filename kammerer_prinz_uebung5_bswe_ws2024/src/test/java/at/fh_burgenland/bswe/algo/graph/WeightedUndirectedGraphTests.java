...
/**
 * This test class contains various test methods for a weighted undirected graph.
 */
class WeightedUndirectedGraphTests {
    ...
    /**
     * This test method verifies that an edge can be added between two vertices and the edge is undirected.
     */
    @Test
    void addEdge() {
        graph.addEdge("C", "A", 15);
        Assertions.assertTrue(graph.hasEdge("C", "A"));
        Assertions.assertTrue(graph.hasEdge("A", "C"));
        Assertions.assertEquals(15, graph.getWeight("C", "A"));
    }

    /**
     * This test method verifies that existing and non-existing edges are correctly identified, also undirected.
     */
    @Test
    void hasEdge() {
